...
# Pressure-level wind products
GEFS_PRESSURE_FILE_ID = "1kaJoW7CLXIzwfB75eQh3Y6tkPW3z3xnA"
ECMWF_PRESSURE_FILE_ID = "1zp-ycSNN3Shk4oGLfAE4eRTCUm29zMoP"

# Height-interpolated wind products
ECMWF_HEIGHT_FILE_ID = "1iVoXZHHS1Zx74JmwwBA-tDgNOCgHTyW6"
APPSTATE_GEFS_FILE_ID = "1h0O90lBOWjkyeqX0sNr77sRHxajVBp3p"
...
def gdrive_download_url(file_id: str) -> str:
    cache_bust = int(pd.Timestamp.utcnow().timestamp() // 300)
    return (
        "https://drive.google.com/uc?"
        f"export=download&id={file_id}&cache_bust={cache_bust}"
    )


def gdrive_csv_url(file_id: str) -> str:
    cache_bust = int(pd.Timestamp.utcnow().timestamp() // 300)
    return f"https://drive.google.com/uc?id={file_id}&cache_bust={cache_bust}"
...
@st.cache_data(ttl=300)
def load_appstate_gefs_summary(file_id: str) -> pd.DataFrame:
    df = pd.read_csv(
        gdrive_csv_url(file_id),
        parse_dates=["time_utc", "time_npt"],
    )
    ...
    return pd.DataFrame(
        {
            ...
            "p25": df["summit_wspd_ms_p25"],
            "p75": df["summit_wspd_ms_p75"],
            "p90": df["summit_wspd_ms_p90"],
            "p_exceed_20": np.nan,
            "p_exceed_25": np.nan,
            "p_exceed_30": np.nan,
            "model": "AppState GEFS summit",
        }
    )
...
def plot_exceedance(df_gefs: pd.DataFrame, df_ecmwf: pd.DataFrame, threshold: int):
    ...
    if np.isfinite(df_gefs[col]).any():
        ax.plot(df_gefs["time_npt"], df_gefs[col], label=df_gefs["model"].iloc[0])

    if np.isfinite(df_ecmwf[col]).any():
        ax.plot(df_ecmwf["time_npt"], df_ecmwf[col], label=df_ecmwf["model"].iloc[0])
...
def plot_vo2_met(df: pd.DataFrame):
    ...
    ax.set_title("ECMWF ENS summit temperature used for VO₂max forecast")
    ax.set_xlabel("Time [Nepal time]")
    ax.grid(True, alpha=0.3)
    ax.legend()
...
wind_mode = st.radio(
    "Wind product",
    [
        "Pressure-level: GEFS 330 hPa + ECMWF 330 hPa",
        "Geometric height: AppState GEFS summit + ECMWF summit",
    ],
    horizontal=False,
)

if wind_mode.startswith("Pressure"):
    required_ids = [GEFS_PRESSURE_FILE_ID, ECMWF_PRESSURE_FILE_ID]
else:
    required_ids = [APPSTATE_GEFS_FILE_ID, ECMWF_HEIGHT_FILE_ID]

if any("PASTE_" in x for x in required_ids):
    st.error("Please replace the Google Drive file IDs for the selected wind product.")
    st.stop()
...
with st.spinner("Loading ensemble wind forecast data..."):
    if wind_mode.startswith("Pressure"):
        ds_gefs = load_dataset(GEFS_PRESSURE_FILE_ID)
        ds_ecmwf = load_dataset(ECMWF_PRESSURE_FILE_ID)

        df_gefs = ensemble_summary(ds_gefs, "GEFS 330 hPa")
        df_ecmwf = ensemble_summary(ds_ecmwf, "ECMWF ENS 330 hPa")

        gefs_init = init_string(ds_gefs)
        ecmwf_init = init_string(ds_ecmwf)

        show_exceedance = True

    else:
        df_gefs = load_appstate_gefs_summary(APPSTATE_GEFS_FILE_ID)
        ds_ecmwf = load_dataset(ECMWF_HEIGHT_FILE_ID)
...
if show_exceedance:
    ...
    st.info(
        "Exceedance-probability plot is currently shown only for the pressure-level products. "
        "The AppState GEFS summary has quantiles but not exceedance probabilities yet."
    )
...
if "PASTE_" not in VO2_FILE_ID:
    ...
    with st.expander("Show temperature used for VO₂max"):
